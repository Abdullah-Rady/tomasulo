...
export const run = (
  ...
  const addLatency = latency.add;
  const subLatency = latency.sub;
  const mulLatency = latency.mul;
  const divLatency = latency.div;
  const loadLatency = latency.load;
  const storeLatency = latency.store;
  const addRSize = size.addSubBuffer;
  const mulRSize = size.mulDivBuffer;
  const storeBufferSize = size.storeBuffer;
  const loadBufferSize = size.loadBuffer;
  const memorySize = size.memory;
  ...
  if (FPR1.length > 0) {
    FPR1.forEach((element) => {
      FPR[parseInt(element.address)] = { qi: 0, val: parseInt(element.value) };
    });
  }
    ...
    for (let i = 0; i < addRS.length; i++) {
      if (addRS[i].status === "ready") {
        ...
        waitingsToWrite.enqueue(
          ...
          { i: priority, j: addRS[i].instQueueIdx }
        );
    ...
    for (let i = 0; i < mulRS.length; i++) {
      if (mulRS[i].status === "ready") {
        ...
        waitingsToWrite.enqueue(
          ...
          { i: priority, j: mulRS[i].instQueueIdx }
        );
    ...
    for (let i = 0; i < loadBuffer.length; i++) {
      ...
      if (loadBuffer[i].status === "ready") {
        ...
        waitingsToWrite.enqueue(
          ...
          { i: priority, j: loadBuffer[i].instQueueIdx }
        );
  ...
  while (finisedItems < instructionQueue.length) {
    ...
    instructionQueueR.push(
      instructionQueue.map((a) => {
        return {
          ...a,
          i: a.executionComplete.i,
          j: a.executionComplete.j,
          d: a.R1 || a.destination,
        };
      })
    );
    FPRR.push(
